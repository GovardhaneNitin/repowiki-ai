...
const App: React.FC = () => {
  ...
  const [staticAnalysisData, setStaticAnalysisData] =
  ...
  const [screenshotAnalysis, setScreenshotAnalysis] =
    useState<ScreenshotAnalysisResult | null>(null);
  ...
  const handleAnalyze = async (skipCache = false) => {
    ...
    if (!skipCache && saved) {
      const parsed = JSON.parse(saved);
      if (
        parsed.repoInfo.name === repoInfo.name &&
        parsed.repoInfo.owner === repoInfo.owner
      ) {
  ...
  const runSandbox = async () => {
    ...
    try {
      const testCmd =
        setupData.test.length > 0 ? setupData.test[0] : "npm test";
...
<pre>${wikiMarkdown}</pre>
  ...
  const ConfidenceBadge: React.FC<{
    ...
    return (
      <span
        title={evidence ? `Source: ${evidence}` : "AI Inferred"}
        className={`ml-2 text-[9px] px-1.5 py-0.5 rounded border uppercase font-bold tracking-wider cursor-help ${colors[level]}`}
      >
        {level} Conf.
      </span>
    );
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 font-sans selection:bg-blue-100 relative flex flex-col">
      {/* Header */}
        ...
        {currentStep === "idle" && !analysisResult && (
          <div className="text-center mt-4 mb-8">
            ...
            <button
              ...
              className="px-3 py-1 bg-white border border-zinc-200 hover:border-blue-300 text-zinc-600 rounded-full text-xs shadow-sm transition-all hover:shadow-md"
            >
            ...
            <div className="grid grid-cols-5 sm:grid-cols-10 gap-1 p-4">
              {stepsList
                ...
                .map((label, idx) => {
                  ...
                  const isDone =
                    currentIndex > stepIndex || currentStep === "complete";
              ...
              <div className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {setupData?.confidence && (
                    <ConfidenceBadge
                      level={setupData.confidence}
                      evidence={setupData.evidence}
                    />
                  )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {setupData?.install?.map((cmd, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 bg-zinc-50 border border-zinc-200 rounded p-2 group"
                      >
                        <code className="text-xs text-pink-600 font-mono flex-1">
                          {cmd}
                        </code>
                        <button
                    ...
                    {(!setupData?.install ||
                      setupData.install.length === 0) && (
                      <span className="text-xs text-zinc-400">
                        No install commands detected
                      </span>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {setupData?.test?.map((cmd, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 bg-zinc-50 border border-zinc-200 rounded p-2"
                      >
                        <code className="text-xs text-blue-600 font-mono flex-1">
                          {cmd}
                        </code>
                        <button
                          onClick={() => copyToClipboard(cmd)}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {setupData.env.map((e, i) => (
                          <span
                            key={i}
                            className="text-[10px] px-2 py-1 bg-yellow-50 text-yellow-800 border border-yellow-200 rounded"
                            title={e.hint}
                          >
                            {e.name}
                          </span>
              ...
              <div className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
                ...
                <div className="space-y-3">
                  {pitfalls.map((p, i) => (
                    <div
                      key={i}
                      className="p-3 rounded-lg border border-zinc-100 bg-zinc-50"
                    >
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  {sandboxResult.failures.length > 0 && (
                    <div className="mb-4">
                      <h5 className="text-xs font-bold text-red-500 mb-1">
                        Failures
                      </h5>
                  ...
                  {sandboxResult.fixes.length > 0 && (
                    <div>
                      <h5 className="text-xs font-bold text-emerald-600 mb-1">
                        Suggested Fixes
                      </h5>
                      <ul className="list-disc list-outside ml-4 text-xs text-zinc-600 space-y-1">
                        {sandboxResult.fixes.map((f, i) => (
                          <li key={i}>{f}</li>
                        ))}
                      </ul>
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">
                      Synthesized from codebase & README
                    </p>
                  </div>
                </div>
                  ...
                  components={{
                    code({ node, inline, className, children, ...props }: any) {
                      return !inline ? (
                        ...
                        <code
                          {...props}
                          className="bg-zinc-100 px-1 py-0.5 rounded text-pink-600 font-mono text-xs"
                        >
                          {children}
                        </code>
                      );
                    ...
                    h1: ({ node, ...props }) => (
                      <h1
                        className="text-3xl font-bold text-zinc-900 mb-6 pb-2 border-b border-zinc-100"
                        {...props}
                      />
                    ),
                    h2: ({ node, ...props }) => (
                      <h2
                        className="text-2xl font-bold text-zinc-800 mt-8 mb-4"
                        {...props}
                      />
                    ),
                    h3: ({ node, ...props }) => (
                      <h3
                        className="text-xl font-semibold text-zinc-800 mt-6 mb-3"
                        {...props}
                      />
                    ),
                    ul: ({ node, ...props }) => (
                      <ul
                        className="list-disc list-outside ml-6 space-y-1 text-zinc-600"
                        {...props}
                      />
                    ),
                    a: ({ node, ...props }) => (
              ...
              {deepScanData?.architecture && (
                <div className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
                  ...
                  <p className="text-sm text-zinc-700 mb-4 font-medium">
                    {deepScanData.architecture.summary}
                  </p>
                  <div className="space-y-2">
                    {deepScanData.architecture.components
                      .slice(0, 3)
                      .map((c, i) => (
                        <div
                          key={i}
                          className="text-xs bg-zinc-50 p-2 rounded border border-zinc-100 text-zinc-600"
                        >
                ...
                <div className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {staticAnalysisData.results.map((res, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center text-xs p-2 hover:bg-zinc-50 rounded cursor-default"
                      >
                        <div className="font-mono font-medium text-zinc-700 truncate max-w-[200px]">
                          {res.path}
                        </div>
                        <div className="text-emerald-600 font-medium">
                          {res.symbols.length} symbols
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">
                    Upload a screenshot of code or a GitHub file for instant
                    analysis
                  </p>
                </div>
              </div>
                ...
                <div className="flex-[2] bg-white rounded-xl border border-zinc-200 p-6 animate-fade-in">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          copyToClipboard(screenshotAnalysis.summary)
                        }
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="bg-zinc-50 p-3 rounded border border-zinc-100">
                        <span className="text-xs text-zinc-400 uppercase font-bold tracking-wider block mb-1">
                          Probable Path
                        </span>
                        <code className="text-xs font-mono text-indigo-600">
                          {screenshotAnalysis.probable_path || "Unknown"}
                        </code>
                      </div>
                      <div className="bg-zinc-50 p-3 rounded border border-zinc-100">
                        <span className="text-xs text-zinc-400 uppercase font-bold tracking-wider block mb-1">
                        ...
                        <p className="text-xs text-zinc-700">
                          {screenshotAnalysis.responsibilities}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-zinc-100 text-right">
                    <span className="text-[10px] text-zinc-400 italic">
                      Image-based understanding powered by Gemini 3 Pro
                      multimodality.
                    </span>
                  </div>
                </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 text-xs">
              <a
                href="https://github.com/GovardhaneNitin/repowiki-ai"
                target="_blank"
                rel="noopener noreferrer"
                className="text-zinc-600 hover:text-blue-600 transition-colors font-medium"
              >
                <GithubIcon size={16} className="inline mr-1" />
                GitHub
              </a>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 animate-fade-in-up relative overflow-hidden">
            ...
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-zinc-900 mb-2">
                Welcome to RepoWiki AI
              </h2>
              <p className="text-zinc-500">
                Turn any GitHub repository into a comprehensive, interactive
                Wiki in seconds using Gemini 3 Pro.
              </p>
            </div>
            ...
            <div className="space-y-4 mb-8">
              <div className="flex gap-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-zinc-800">
                    Paste URL or use Demo
                  </h4>
                  <p className="text-xs text-zinc-500">
                    Enter a public GitHub URL (e.g. facebook/react) or click the
                    "Demo" button.
                  </p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">
                    Gemini scans file structure, architecture, and pitfalls.
                  </p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">
                    Download the generated Wiki as Markdown or HTML.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => setShowIntroModal(false)}
                className="flex-1 py-3 bg-zinc-900 hover:bg-zinc-800 text-white rounded-lg font-medium transition-colors"
              >
                Get Started
              </button>
              <button
                onClick={loadDemoData}
                className="flex-1 py-3 bg-white border border-zinc-200 hover:bg-zinc-50 text-zinc-700 rounded-lg font-medium transition-colors"
              >
                Load Demo Repo
              </button>
            </div>
